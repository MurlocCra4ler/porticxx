...
public:
    ...
    exception& operator*() const noexcept { return *m_ptr; }

    exception* operator->() const noexcept { return m_ptr.get(); }
...
template <class E> constexpr exception_ptr make_exception_ptr(E e) noexcept {
    static_assert(is_base_of_v<exception, E>,
                  "E must derive from std::exception");
