...
static inline void* sys_mmap(size_t len) {
    long r = sys_call6(SYS_MMAP, reinterpret_cast<long>(nullptr),
                       static_cast<long>(len), PROT_READ | PROT_WRITE,
                       MAP_PRIVATE | MAP_ANONYMOUS | MAP_STACK, -1, 0L);
    return (void*)r;
}

static inline int sys_munmap(void* addr, size_t len) {
    long ret = sys_call3(SYS_UNMAP, reinterpret_cast<long>(addr),
                         static_cast<long>(len), 0);
...
static inline int sys_futex(int* uaddr, int futex_op, int val,
    ...
    return static_cast<int>(sys_call6(
        SYS_FUTEX, reinterpret_cast<long>(uaddr), static_cast<long>(futex_op),
        static_cast<long>(val), reinterpret_cast<long>(timeout),
        reinterpret_cast<long>(uaddr2), static_cast<long>(val3)));
}

int futex_wait(int* addr, int val) { return sys_futex(addr, FUTEX_WAIT, val); }

extern "C" long _sys_clone(long flags, long child_sp, long parent_tid,
                           long child_tid, long);
...
x86_64_arch::thread_handle_type
x86_64_arch::thread_create(void* (*start)(void*), void* arg) {
    // create stack
    constexpr size_t stack_size = 1ull << 20;
    void* stack = sys_mmap(stack_size);

    uintptr_t* tos = reinterpret_cast<uintptr_t*>(
        reinterpret_cast<uint8_t*>(stack) + stack_size);
    tos = reinterpret_cast<uintptr_t*>(reinterpret_cast<uintptr_t>(tos) &
                                       ~0xfull);
    tos -= 4;
    ...
    *child_tid = 1;
    ...
    long rc =
        _sys_clone(static_cast<long>(flags), reinterpret_cast<long>(child_sp),
                   0L, reinterpret_cast<long>(child_tid), 0L);
...
void x86_64_arch::thread_join(x86_64_arch::thread_handle_type h,
                              void** retval) {
